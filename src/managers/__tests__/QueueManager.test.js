...
describe('QueueManager', () => {
  ...
  describe('add()', () => {
    test('대기열에 아이템을 추가하고 순서를 반환해야 함', () => {
      // Given: 빈 대기열
      // When: 아이템 추가
      const position = queueManager.add('client-1', 'worry-1', jest.fn());
    ...
    test('여러 아이템을 추가하면 순서대로 순서를 반환해야 함', () => {
      // Given: 빈 대기열
      // When: 3개 아이템 순차 추가
      const pos1 = queueManager.add('client-1', 'worry-1', jest.fn());
      const pos2 = queueManager.add('client-2', 'worry-2', jest.fn());
      const pos3 = queueManager.add('client-3', 'worry-3', jest.fn());
    ...
    test('타임아웃 발생 시 콜백이 호출되어야 함', () => {
      ...
      queueManager.add('client-1', 'worry-1', onTimeout);

      // When: 5분 경과 (타임아웃)
      jest.advanceTimersByTime(5 * 60 * 1000);

      // Then: 콜백이 호출되고 대기열에서 제거됨
      expect(onTimeout).toHaveBeenCalledWith('client-1');
      expect(queueManager.getLength()).toBe(0);
    });
  });
  ...
  describe('remove()', () => {
    test('대기열에서 아이템을 제거할 수 있어야 함', () => {
      ...
      queueManager.add('client-1', 'worry-1', jest.fn());
      queueManager.add('client-2', 'worry-2', jest.fn());

      // When: 첫 번째 아이템 제거
      const removed = queueManager.remove('client-1');
    ...
    test('존재하지 않는 clientId는 false를 반환해야 함', () => {
      // Given: 빈 대기열
      // When: 존재하지 않는 clientId 제거 시도
      const removed = queueManager.remove('client-999');
    ...
    test('제거 후에도 남은 아이템은 유지되어야 함', () => {
      ...
      queueManager.add('client-1', 'worry-1', jest.fn());
      queueManager.add('client-2', 'worry-2', jest.fn());
      queueManager.add('client-3', 'worry-3', jest.fn());

      // When: 중간 아이템 제거
      queueManager.remove('client-2');

      // Then: 나머지 아이템은 유지되고 순서도 유지됨
      expect(queueManager.getLength()).toBe(2);
      expect(queueManager.getPosition('client-1')).toBe(1);
      expect(queueManager.getPosition('client-3')).toBe(2);
    });
  });
  ...
  describe('dequeue()', () => {
    ...
    test('dequeue 후에도 타임아웃은 계속 작동해야 함', () => {
      ...
      const onTimeout = jest.fn();
      queueManager.add('client-1', 'worry-1', jest.fn());
      queueManager.add('client-2', 'worry-2', onTimeout);
      ...
      expect(onTimeout).toHaveBeenCalledWith('client-2');
    });
  });

  describe('getPosition()', () => {
    test('clientId의 순서를 조회할 수 있어야 함', () => {
      queueManager.add('client-1', 'worry-1', jest.fn());
      queueManager.add('client-2', 'worry-2', jest.fn());
      queueManager.add('client-3', 'worry-3', jest.fn());

      expect(queueManager.getPosition('client-1')).toBe(1);
      expect(queueManager.getPosition('client-2')).toBe(2);
      expect(queueManager.getPosition('client-3')).toBe(3);
    });

    test('존재하지 않는 clientId는 null을 반환해야 함', () => {
      expect(queueManager.getPosition('client-999')).toBeNull();
    });
  });

  describe('getAllClientIds()', () => {
    test('모든 clientId를 배열로 반환해야 함', () => {
      queueManager.add('client-1', 'worry-1', jest.fn());
      queueManager.add('client-2', 'worry-2', jest.fn());
      queueManager.add('client-3', 'worry-3', jest.fn());

      const clientIds = queueManager.getAllClientIds();
      expect(clientIds).toEqual(['client-1', 'client-2', 'client-3']);
    });
  ...
  describe('clear()', () => {
    ...
    test('clear 후에는 타임아웃이 발생하지 않아야 함', () => {
      ...
      queueManager.add('client-1', 'worry-1', onTimeout1);
      queueManager.add('client-2', 'worry-2', onTimeout2);
  ...
  describe('복합 시나리오', () => {
    test('추가 → 제거 → 순서 업데이트가 정상적으로 작동해야 함', () => {
      ...
      expect(queueManager.getPosition('client-1')).toBe(1);
      expect(queueManager.getPosition('client-3')).toBe(2);
      expect(queueManager.getPosition('client-2')).toBeNull();
    });
    ...
    test('dequeue → add → dequeue가 정상적으로 작동해야 함', () => {
      ...
      expect(third.clientId).toBe('client-3');
    });
